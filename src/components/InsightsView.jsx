...
export default function InsightsView({
  ...
  if (!data && loading) return <LoadingDashboard />;

  if (!data && error) {
    return (
      <section aria-labelledby="insights-title" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        <InsightsHeader range={range} onChangeRange={onChangeRange} headingRef={headingRef} />
  ...
  return (
    <section aria-labelledby="insights-title" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <InsightsHeader range={range} onChangeRange={onChangeRange} data={data} loading={loading} headingRef={headingRef} />
      {error ? (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(230px, 0.8fr) minmax(460px, 1.7fr)', gap: 12, alignItems: 'stretch' }}>
