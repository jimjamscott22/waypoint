...
**Discovery providers:** a provider is `{ id, planRoleFamily, search }`. `id` must equal the `provider` value its normalizer stamps on listings and be registered in `server/providers.js`, which is the only place a provider's display label lives — never hardcode a provider name in a repository or route. `planRoleFamily({ query, roleFamily })` returns request descriptors and is where a provider decides how to decompose a search; `searchRoleFamily` spreads each descriptor into `search`, so descriptor fields are that provider's own vocabulary. Each provider gets its own slice of the request budget (`server/discovery/budget.js`) so one cannot starve another. Anything identifying a listing must key on `provider` **and** `providerJobId` — the latter is unique only within a provider.
